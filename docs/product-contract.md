...
| Update note modal | Shows version/update announcement. | First visit without `prefrontal_lab_v6.2.1_update`, or URL has `showUpdate`. | User dismisses and app stores seen flag. | Not shown while analytics page is active. |
...
| `prefrontal_lab_v6.2.1_update` | string flag | no | Update note visibility | Browser `localStorage` | Stored after user closes update note. |
...
| Update note | None currently. | Hidden after seen flag exists. | None currently. | None currently. | Close stores `prefrontal_lab_v6.2.1_update`. |
