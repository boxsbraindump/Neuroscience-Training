...
- Main workflow: user opens app, chooses language and mode, starts a task or arena run, completes or abandons a session, and sees score/result.
...
| --- | --- | --- | --- | --- |
| Home | Landing surface for score, language, mode, and task selection. | Default `view = home`. | User starts one training task or arena mode. | Has zh/en toggle, best score card, mode tabs, task cards, and owner analytics shortcut when unlocked. |
| Update note modal | Shows version/update announcement. | First visit without `prefrontal_lab_v6.1.1_update`, or URL has `showUpdate`. | User dismisses and app stores seen flag. | Not shown while analytics page is active. |
...
| `prefrontal_lab_v6.1.1_update` | string flag | no | Update note visibility | Browser `localStorage` | Stored after user closes update note. |
...
| `analytics_events.task` | text | no | Top task / clicks | Frontend event payload | Task key or `arena`. |
| `analytics_events.mode` | text | no | Aggregated indirectly | Frontend event payload | `normal`, `hard`, or `comp`. |
...
| --- | --- | --- | --- | --- | --- |
| Home | None currently. | Task list is always generated from `TASK_DATA`. | None currently. | Hard tab is disabled until `isHardUnlocked`. | Task card starts a run; mode selection updates immediately. |
| Update note | None currently. | Hidden after seen flag exists. | None currently. | None currently. | Close stores `prefrontal_lab_v6.1.1_update`. |
| Task info | None currently. | Hidden when `showInfo` is null. | None currently. | None currently. | Modal displays selected task guide. |
| Game top bar | Timer active while in game. | N/A. | Wrong answer can flash timer/error state. | Buttons may be disabled while feedback is shown. | Score increments and task advances. |
| Schulte | N/A. | N/A. | Wrong number flashes error / arena penalty. | N/A. | Correct sequence advances; final number ends solo task or rotates arena. |
