...
| Update note modal | Shows version/update announcement. | First visit without `prefrontal_lab_v6.2.0_update`, or URL has `showUpdate`. | User dismisses and app stores seen flag. | Not shown while analytics page is active. |
...
| `pfl_appearance` | string | no | Whole app | Browser `localStorage` | `system` (default), `light`, or `dark`. Same key name as the iOS app. Read twice: by a boot script in `index.html` before first paint, so a dark-mode visitor never sees a white flash, and by `App.jsx`, which applies the `theme-dark` class to `<html>` and follows the OS live while set to `system`. All dark styling lives in `src/theme-dark.css` and is scoped to that class, so light mode is unchanged. |
...
| `prefrontal_lab_v6.2.0_update` | string flag | no | Update note visibility | Browser `localStorage` | Stored after user closes update note. |
...
| Update note | None currently. | Hidden after seen flag exists. | None currently. | None currently. | Close stores `prefrontal_lab_v6.2.0_update`. |
| Task info | None currently. | Hidden when `showInfo` is null. | None currently. | None currently. | Modal displays selected task guide. |
| Game top bar | Timer active while in timed games; Endless shows `∞`; Code Logic shows elapsed time and, in Endless, the puzzle number. | N/A. | Wrong answer can flash timer/error state. | Buttons may be disabled while feedback is shown. | Score increments and task advances. |
...
| Code Logic | N/A. | Slots start empty and the first one is selected. | Incomplete entry and wrong code each show a message for under a second; a wrong submit costs 10 points and keeps the entry for editing. | Submit is ignored while the solved message is showing. | Correct submit ends the run, or draws the next puzzle in Endless. |
| Result | N/A. | N/A. | N/A. | N/A. | Shows score feedback and persists history. Code Logic replaces the five-band verdict and accuracy with completed / solve time / incorrect submissions, because one puzzle is worth at most 100 points. |
